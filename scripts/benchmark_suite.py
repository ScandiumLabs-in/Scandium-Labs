...
def fluorite(cation, anion, a):
    return Structure(Lattice.cubic(a), [cation] + [anion]*2, [[0,0,0], [0.25,0.25,0.25], [0.75,0.75,0.75]])

def antifluorite(anion, cation, a):
    return Structure(Lattice.cubic(a), [anion] + [cation]*2, [[0,0,0], [0.25,0.25,0.25], [0.75,0.75,0.75]])
...
def nickelates(rare_earth, a):
    return Structure(Lattice.orthorhombic(a, a*0.98, a*1.02),
        ...
        [[0.5,0,0.25],[0,0.5,0.25],[0.5,0,0.75],[0,0.5,0.75],
         [0.75,0.5,0],[0.25,0.5,0],[0.75,0.5,0.5],[0.25,0.5,0.5],
         [0.5,0.75,0],[0.5,0.25,0],[0.5,0.75,0.5],[0.5,0.25,0.5]])

def layered_li_co_oxide(a, c):
    return Structure(Lattice.hexagonal(a, c),
        ['Li', 'Co', 'O', 'O'],
...
MATERIALS = [
    ...
    ("Li2S_li_superionic", Structure(Lattice.cubic(5.72),
        ['Li', 'Li', 'S'],
...
def main():
    ...
    print(f"Model: hidden_dim={engine.model.hidden_dim}, "
