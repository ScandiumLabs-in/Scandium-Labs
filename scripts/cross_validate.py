...
def create_folds(all_graphs, n_folds=5, seed=42):
    ...
    prefix_counts = {p: prefixes.count(p) for p in unique_prefixes}
...
def compute_metrics(y_true, y_pred):
    ...
    r2 = r2_score(yt, yp)
    return {'mae': float(mae), 'rmse': float(rmse), 'r2': float(r2), 'n': int(mask.sum())}
...
def train_fold(fold_id, train_idx, val_idx, all_graphs):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        maes = ' | '.join(
            f'{t}: {epoch_metrics[t]["mae"]:.4f}' if epoch_metrics[t]['mae'] is not None else f'{t}: n/a'
            for t in tasks
        )
        print(f'  Epoch {epoch:3d}: train_loss={avg_train_loss:.4f} val_loss={avg_val_loss:.4f} | {maes} | {elapsed:.0f}s')
...
def main():
        ...
        for metric in ['mae', 'rmse']:
            if stats.get(metric) and t_std:
                ...
                stats[f'{metric}_physical'] = {
                    ...
                    'ci95': ci95_phys,
                    'values': vals_phys,
                    'unit': {'formation_energy': 'eV/atom', 'energy_above_hull': 'eV/atom', 'band_gap': 'eV'}.get(task, ''),
                }
    ...
    for fs in summary['folds']:
        print(f'{fs["fold_id"]+1:>6d} {fs["total_epochs"]:>7d} {fs["total_time_s"]:>8.0f}', end='')
        for task in tasks:
