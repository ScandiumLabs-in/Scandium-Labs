...
def parse_args():
    ...
    parser.add_argument("--target", type=int, default=10000, help="Target number of structures")
    parser.add_argument(
        "--output",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--skip-download", action="store_true", help="Skip download, use existing cache"
    )
...
def download_from_jarvis(target):
    print("[JARVIS] Loading dft_3d dataset...")
...
def print_report_summary(report):
    ...
    print(
        f"  Unique formulas: {report['formulas']['n_unique']} ({report['formulas']['ratio_unique']:.1%})"
    )
    ...
    for t, stats in report["targets"].items():
        ...
        print(
            f"    {t:30s}: {stats['n_labeled']:6d}/{report['n_structures']} ({pct:5.1f}%)"
            f"  mean={stats['mean']}"
        )
    print(f"{'=' * 60}\n")
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print("  Scandium Labs Dataset Builder")
    ...
    api_key = (
        args.api_key or os.environ.get("MP_API_KEY") or os.environ.get("MATERIALS_PROJECT_API_KEY")
    )
    ...
    n = len(structures)
    if n == 0:
        print("  Warning: no structures to split")
        ...
        dummy_dataset = type(
            "Dummy",
            (),
            {"structures": structures, "__len__": lambda self: len(self.structures)},
        )()
    ...
    if args.cache_graphs and structures:
        ...
        print(
            f"  Built {len(all_graphs)} graphs in {elapsed:.1f}s ({elapsed / len(all_graphs):.2f}s/graph)"
        )
