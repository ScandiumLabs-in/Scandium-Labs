...
from src.inference.stability import hull_consistency_flag, resolve_stability
from src.training.data_audit import gate_predictions, STATUS_NO_LABELS, STATUS_MC_DISABLED
...
class InferenceEngine:
    def __init__(self, model_path, device='cuda', use_mc_dropout=True, mc_samples=20):
        ...
        self.graph_builder = ALIGNNGraphBuilder(cutoff=8.0, max_neighbors=32,
                                                num_sbf=num_sbf)
    ...
    def _build_coverage_report(self):
        ...
        from src.training.data_audit import REQUIRED_TASKS, MIN_VIABLE_LABELS
    ...
    def predict_single(self, structure, temperature=300.0):
        ...
        if self.use_mc_dropout:
            ...
            predictions = {
                task: {'value': pred.item(), 'uncertainty': None}
                for task, pred in raw_preds.items()
            }

        predictions = self.normalizer.denormalize(predictions)
        predictions = gate_predictions(predictions, self._coverage_report)

        for task, pred in predictions.items():
            if isinstance(pred, dict) and pred.get('value') is not None and pred.get('uncertainty') is None:
        ...
        if 'log_ionic_conductivity' in predictions:
            ...
            if log_val is None:
                ...
                if raw_log_std is not None:
                    sigma_uncertainty = 10 ** (log_val + raw_log_std) - 10 ** (log_val - raw_log_std)
                else:
        ...
        if self.ood_detector:
            with torch.no_grad():
                embedding = self.model.pool(
                    self.model.encode(crystal_graph, line_graph),
                    crystal_graph
                ).cpu().numpy()
        ...
        if fe is not None and eah is not None:
            ...
            predictions['stability_check'] = {'suspicious': False, 'reason': 'insufficient data'}
    ...
    def _stability_bands(eah):
        ...
        if eah < 0.20:
            return {'label': 'Potentially synthesizable', 'color': 'orange', 'icon': '🟠'}
